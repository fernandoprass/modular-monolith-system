...
-   **GetByIdAsync(TId id, CancellationToken cancellationToken)**: Retrieves a single entity by its primary key.
-   **AddAsync(T entity, CancellationToken cancellationToken)**: Adds a new entity to the tracking context.
-   **Update(T entity)**: Marks an existing entity as modified.
-   **DeleteAsync(TId id, CancellationToken cancellationToken)**: Removes an entity from the database.
-   **ExistsAsync(TId id, CancellationToken cancellationToken)**: Checks for the existence of a record without loading the full object into memory.
