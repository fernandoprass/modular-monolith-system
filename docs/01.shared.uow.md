...
The `UnitOfWork<TContext>` base class intercepts the `SaveChangesAsync` call. Before persisting data, it iterates through the change tracker for any class inheriting from `EntityAudited`:
