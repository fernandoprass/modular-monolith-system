...
* **Automation:** Our `iam.setup-db.ps1` script generates this file automatically if missing.
...
* **External Access:** Accessible in your browser at `http://localhost:8080` or `http://127.0.0.1:8080` .
...
3.  **Connection Tab:**
    * **Host name/address:** Type `postgres_db` (This is the service name defined in your `docker-compose.postgresql.yaml`. Docker's internal DNS resolves this name to the correct container IP).
    * **Port:** `5432`
