...
- `src/03.IAM/`: The IAM (Identity and Access Management) module.
- `src/01.Shared/`: Common utilities packaged as NuGet packages for reuse across modules.
...
- endpoints for client interactions.
  ...
  - `Controllers/`: API controllers like `UserController`, `OrganizationController`, `PermissionController`, `ParameterController`.
  - `Middlewares/`: Custom middleware and authorization helpers like `RequirePermissionAttribute` and `PermissionAuthorizationHandler`.
...
- **`IAM.Domain/`**: The innermost layer containing domain entities, value objects, and interfaces. This layer is independent of any external frameworks and represents the core business rules and data structures.
  - `Organization.cs`, `User.cs`: Domain entities.
...
- **`IAM.Infrastructure/`**: Handles external concerns like databases, APIs, and third-party services. This layer implements interfaces defined in Domain and Core.
  ...
  - `DatabaseSeeder/`: Development seeders for organizations, parameters, permissions, roles, and role-permission assignments.
...
Role: It should remain "thin," performing no business logic. Its main job is to extract the Operator's Context (like the operatorOrganizationId 
from the JWT) and **pass the DTO and CancellationToken to the Orchestrator or Service**.

#### 2. RegisterOrchestrator (The Coordinator)
The Orchestrator acts as a "Maestro" for operations that cross domain boundaries. For example, creating a user requires checking if a Organization 
exists and if the User's email is unique.

Purpose: To coordinate logic between different domains (User and Organization) and enforce high-level security rules, such as ForbiddenOrganizationError.
...
Role: It receives the request DTO and "facts" provided by the Orchestrator (e.g., emailExists, organizationExists) and returns a Result object 
...
sequenceDiagram
    ...
    UserController->>RegisterOrchestrator: RegisterUserAsync(Request, OperatorId, cancellationToken)
    end
 
    rect rgb(230, 245, 255)
    Note over RegisterOrchestrator: Cross-Domain Logic
    RegisterOrchestrator->>RegisterOrchestrator: Security Check (ForbiddenOrganizationError)
    RegisterOrchestrator->>QueryRepo: Fetch Organization & Email status(cancellationToken)
    ...
    RegisterOrchestrator->>UserService: CreateUserAsync(Request, cancellationToken)
    UserService->>Domain: User.Create(...)
    UserService->>UoW: Users.AddAsync(user, cancellationToken)
    UserService->>UoW: SaveChangesAsync(cancellationToken)
...
## Shared Module
- `src/01.Shared/`: Common utilities as a NuGet package.
